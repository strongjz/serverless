...
const isImplicitlyInstallable = require('../../utils/isExecutable') && process.platform !== 'win32';
...
module.exports = class Executable {
  constructor(serverless) {
    ...
    this.commands = {
      upgrade: {
        isHidden: !isImplicitlyInstallable,
    ...
    this.hooks = {
      'upgrade:upgrade': () => {
        return isImplicitlyInstallable ? this.upgrade() : this.rejectCommand('upgrade');
      },
      'uninstall:uninstall': () => {
        return isImplicitlyInstallable ? this.uninstall() : this.rejectCommand('uninstall');
      },
    };
  }
