...
const fse = BbPromise.promisifyAll(require('../../../utils/fs/fse'));
...
const resolveNpmBinPath = _.memoize(() => {
  const localNpmBinPath = path.join(serverlessPath, 'node_modules/npm/bin/npm-cli.js');
  return fse.statAsync(localNpmBinPath).then(
    stats => {
      return stats.isFile() ? localNpmBinPath : null;
    },
    error => {
...
class PluginInstall {
  ...
  npmInstall(name) {
    return resolveNpmBinPath().then(localNpmBinPath => {
      return childProcess.execAsync(
        `${localNpmBinPath ? `node ${localNpmBinPath}` : 'npm'} install --save-dev ${name}`,
        {
          stdio: 'ignore',
        }
      );
